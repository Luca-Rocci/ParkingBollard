...
module barriera(h,l,n) {
    ...
    hb = (l-240)/(n-1);
    hbb = l/(n+1);
    mirror([1,0,0]) paletto_singolo(hp,n+2,0);
    translate([120,0,hp + 12]) rotate([90,0,180]) curva(16,90);
    translate([l,0,0]) paletto_singolo(hp,n+2,0);
    ...
    union() {
        for (i = [0 : n]) {
            let (x = i*hbb, c = i%2==1 ? BandaRossa : PvcBianco) {
                translate([x,0,(h/2)-20])
                    color(c) rotate([90,0,90])
                        cylinder_tube(hbb, 16, 1);
            }
        }
    }
}
